...
/**
 * The slots a Blueprint author declared on a supplied page, compiled to byte
 * spans in that page's markup.
 *
 * Ported from Wesley Fantinel's `ApprovedPattern`. This is the strict half of
 * personalization: only a declared slot changes, everything else keeps its
 * approved bytes, and a declaration that cannot be honoured safely (a path
 * that points nowhere, a protected block, rich text that a plain string would
 * flatten, two slots on one span) refuses the page before any model runs.
 * `ContentSlots` is the other half, where slots are discovered from the markup
 * and `ai-ignore` is the opt-out.
 *
 * Serialization replaces only the compiled spans. It does not parse and
 * reserialize surrounding blocks, so wrappers, styles, comments and siblings
 * keep their exact bytes. Where a block mirrors sourced text in its comment
 * JSON, that string is kept in step without touching the other keys.
 */
final class DeclaredSlots
{
