...
final class EarlyImageBuild
{
    ...
    private function start(): void
    {
        ...
        foreach (glob($this->storage . '/*', GLOB_ONLYDIR) ?: [] as $directory) {
    ...
    /** Keep paid raw results and attempt logs even if a later graph step fails. */
    public function finishRaw(bool $publish = false): void
    {
        try {
            ...
            if ($publish && $this->directory !== null) {
    ...
    {
        $sources = glob($this->storage . '/*/logs/attempts.jsonl') ?: [];
        if ($sources === []) {
            return;
        }
        ...
        try {
            ...
            foreach ($sources as $source) {
                foreach (file($source, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) ?: [] as $index => $line) {
                    $id = hash('sha256', $source . ':' . $index);
                    $record = json_decode($line, true);
                    if (isset($seen[$id]) || !is_array($record)) {
                        continue;
                    }
