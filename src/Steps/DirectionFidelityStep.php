...
final class DirectionFidelityStep implements Step
{
    ...
    /**
     * Put the committed device class on the first non-hero band when the
     * model forgot. HTML-first convert often ships `tagName:section` without
     * `alignfull`, so the stamp accepts either a full-bleed class or a
     * section-tagged group.
     *
    ...
    {
        $class = Device::className(Device::explicit($direction['device'] ?? null));
        ...
        $rewritten = preg_replace_callback(
            ...
            static function (array $match) use ($class, &$stamped): string {
                if ($stamped || str_contains($match[2], 'hero-composition')) {
                    return $match[0];
                }
                if (!str_contains($match[2], 'alignfull') && !str_contains($match[2], 'wp-block-group')) {
                    return $match[0];
                }
                $stamped = true;
        ...
        if (!is_string($rewritten) || !$stamped) {
            $rewritten = preg_replace_callback(
                '/(<!-- wp:group\s*\{)([^}]*)(\})/',
                static function (array $match) use ($class, &$stamped): string {
                    if ($stamped || str_contains($match[0], 'hero-composition')) {
                        return $match[0];
                    }
                    if (
                        !str_contains($match[2], 'alignfull')
                        && !str_contains($match[2], '"tagName":"section"')
                    ) {
                        return $match[0];
                    }
                    $stamped = true;
                ...
                is_string($rewritten) ? $rewritten : $markup,
            );
        }
        if (!is_string($rewritten) || !$stamped) {
            return [$markup, []];
        ...
        $stampedHtml = false;
        $rewritten = preg_replace_callback(
            '/(<(?:section|div)\b[^>]*\bclass=")([^"]*)(")/i',
            static function (array $match) use ($class, &$stampedHtml): string {
                if ($stampedHtml
                    || str_contains($match[2], 'hero-composition')
                    || str_contains($match[2], 'hero-media')
                ) {
                    return $match[0];
                }
                $isSection = str_starts_with(strtolower($match[0]), '<section');
                if (!$isSection && !str_contains($match[2], 'alignfull')) {
                    return $match[0];
                }
    ...
    {
        $surface = Surface::explicit($direction['surface'] ?? null) ?? Surface::DEFAULT;
        if ($surface === 'none') {
    ...
    {
        $device = Device::explicit($direction['device'] ?? null) ?? Device::DEFAULT;
        $class = Device::className($device);
    ...
    public static function repairMotion(string $markup, array $direction): array
    {
        ...
        $mapped = Motion::mapNote($direction['motion_note'] ?? '', $profile);
        if (in_array('hover-lift', $mapped['classes'], true) && !str_contains($rewritten, 'hover-lift')) {
