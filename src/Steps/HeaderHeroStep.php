...
final class HeaderHeroStep implements Step
{
    ...
    public static function fixHeader(
        ...
        if (in_array($archetype, ['floating-pill', 'bar-center-cta', 'spread-nav'], true)) {
            $marked = match ($archetype) {
                'floating-pill' => HeaderNav::withPillRow($markup),
                'bar-center-cta' => HeaderNav::withBarCenterRow($markup),
                default => HeaderNav::withSpreadRow($markup),
            };
    ...
    private static function capOpeningCovers(
        ...
        self::eachOpeningPart(
            ...
            $writes,
            $report,
            static fn (string $markup): array => \Automattic\SiteBuild\HeroComposition::isAuthoredMarkup($markup)
                ? ['markup' => $markup, 'notes' => []]
                : self::capCovers($markup),
        );
    }
