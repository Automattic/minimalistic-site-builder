    ...
    public static function format(array $direction): string
    {
        ...
        if ($device !== null && $device !== 'none' && $deviceClass !== null) {
            $deviceMeaning = match ($device) {
                'hairline-rule'   => 'a 3px accent inset rule on ONE non-hero section root',
                'section-numeral' => 'a folio numeral on ONE non-hero section root',
                'stamp'           => 'a small rotated stamp on ONE non-hero section root',
                default           => 'the committed CSS device',
            };
            $facts[] = "- **Device**: {$device} — place `{$deviceClass}` on at most one non-hero `<section>`. {$deviceMeaning}. Never on the hero. The build stamps the class if the page forgets.";
        }
