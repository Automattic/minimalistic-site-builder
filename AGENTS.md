...
The `sections` step runs in content mode by default: `SectionUnit` and `HeroUnit` send `prompts/section-content.md` with one JSON schema (`SectionContent::schema()`), the model returns a content document (copy, items, image subjects, one action label), and `SectionContentCompiler` / `HeroContentCompiler` compile it into the block markup the markup prompts ask for — same root markers, hooks, surfaces, and `AI_IMAGE` placeholders — before the usual `finish()` repair chain runs. Every section and the front hero share the same cached site, build, and page layers and the same schema (the API folds the output schema into the cache key). `SITE_BUILD_SECTION_OUTPUT=markup` selects the markup prompts (`prompts/section.md`, `prompts/hero.md`). The header and footer keep their markup prompts in both modes. See `plan/sections-content-compiler.md`.
