...
`SITE_BUILD_SECTION_OUTPUT=content` switches the `sections` step's page sections to content mode: `SectionUnit` sends `prompts/section-content.md` with a JSON schema (`SectionContent::schema()`), the model returns a content document (copy, items, image subjects, one action label), and `SectionContentCompiler` compiles it into the same block markup the markup prompt asks for — same root markers, hooks, surfaces, and `AI_IMAGE` placeholders — before the usual `SectionUnit::finish()` repair chain runs. The header, footer, and hero keep their markup prompts. Unset or any other value keeps the markup prompt (`prompts/section.md`). See `plan/sections-content-compiler.md`.
