...
Pass `--html-first` (or set `SITE_BUILD_GRAPH=html-first`) for the HTML-first graph (`StepComposition::htmlFirst()`), where the model authors an HTML+CSS design that `transform-site` converts to blocks:
