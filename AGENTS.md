...
Pass `--tree` (or set `SITE_BUILD_GRAPH=tree`) for the tree graph (`StepComposition::tree()`) — the x-pipeline brochure port. The model authors JSON block trees (`{name, attributes, innerBlocks}` — never serialized markup) against a live Playground sandbox; the `sandbox/companion/` plugin validates every tree against the real block registry and hosts the harness page where the site's own `wp.blocks` compiles them (serialize → re-parse → content-parity), design tokens land as user-origin global styles on the default theme (the theme's own spacing/layout pass through byte-for-byte), and pages publish straight into the sandbox, which stays running after the build as the finished site (`php bin/sandbox.php <slug> --stop` stops it):

`sandbox -> brief -> read-instance -> tokens -> section-trees -> tree-repair -> publish -> tree-images -> verify`

A tree build makes no custom theme, no custom blocks and no schema packages (brochure: composition only). The brief fixes the call budget up front (`budget.json` + `logs/tree-ledger.jsonl`; a `--from` resume continues the same bill). `tree-images` is in-graph but a no-op unless `meta.json` records `tree_images` (the CLI's `--with-images`); without it every image slot ships a 1×1 placeholder GIF carrying its written `imageIntent`. `verify` measures the rendered site (heading outline, band widths and seams, text contrast, image loading) and records findings in `warnings.json` per the escalation ladder. `bin/sandbox/harness.mjs` (playwright-core against system Chrome, like `bin/screenshot`) is the browser side; the sandbox companion's routes only answer where the boot blueprint defined `MSB_COMPANION_SANDBOX`. See `plan/tree-graph-port-plan.md` for the port's source mapping.

`createProject()` records the chosen graph in `meta.json` as `graph: tree|html-first|blocks`, and `--from` reads it back so a resume runs the graph that built the project. A flag contradicting the record is refused, not honored — `section-rhythm`, `collect-images` and friends exist in both graphs, so a crossed resume would otherwise read artifacts the other path never wrote and still look like it worked. A project with no recorded graph (built before this landed) falls back to the flag/env choice. A host driving a fixed `StepComposition` must pass `htmlFirst:` to `createProject()`, since the default reads the env key that host never consults.
