...
- The build supplies global background/text and body typography wiring (including `styles.typography.textWrap: "pretty"` so headings and paragraphs never end on a dangling last word), h1–h6 and caption role wiring, family/size wiring for exactly these blocks: `core/quote`, `core/pullquote`, `core/table`, `core/list`, `core/image`, `core/site-title`, and `core/navigation`, and the committed image/button corner language described above. Do not emit `styles.typography.textWrap` and do not set `textWrap` on heading or paragraph elements — the build owns that leaf.
