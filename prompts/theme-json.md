...
- **Heading line height.** This remains a model-authored design choice. Choose 1.1–1.3; never below 1.0, and set it at `styles.elements.heading.typography.lineHeight` — the build supplies the body line-height but not this one, so headings inherit a body rhythm unless you set it. The DESIGN DIRECTION's **Type treatment** owns `textTransform` and `letterSpacing` at that same path. Do not emit either owned leaf on `heading`, `h1`–`h6`, `core/heading`, a variation, or custom CSS; the build writes the committed pair and removes structured per-level conflicts while preserving every line-height choice.
