...
- **Type size scale — modest body, one masthead-scale top step.** Body 1rem; section headings scale modestly (h2 ~1.75–2.25rem, h3 below it). The TOP step of the scale is the hero masthead size: a fluid `clamp()` reaching roughly 5–7rem at desktop widths — size it to the DESIGN DIRECTION's ambition (a broadsheet/poster direction earns ~7rem; a quiet editorial one ~5rem). Do NOT cap it near 3.5–4rem — an undersized hero headline reads as timid. The step below the top stays around 2.5–3.5rem. Encode a 6-step scale onto `settings.typography.fontSizes` (give each a slug + name), e.g. `0.875rem / 1rem / 1.25rem / 1.75rem / clamp(2.25rem, 3vw, 3rem) / clamp(3rem, 7vw, 6rem)`. Each step has a ROLE — caption/metadata, body, lead line, heading, section title, masthead — name the entries accordingly. Only the two smallest steps are paragraph sizes (the third, ~1.25rem, is for a single short lead line at most); everything above is heading territory, and the top step exists for ONE hero/masthead moment per page.
...
- settings.typography.fontSizes: a 6-step scale (each entry { "slug", "name", "size" }) following the scale above. The LARGEST step MUST use the slug "display" — sections reference the hero masthead size by that exact slug.
...
- styles.elements.h1 / h2 / h3 typography.fontFamily = heading font var, with a clear size hierarchy drawn from the fontSizes scale via `var(--wp--preset--font-size--<slug>)` references: h1 = the "display" step (it renders once per page, as the hero masthead), h2/h3 = modest middle steps.
