...
- The same ban covers TEXT-BEARING PROPS as a prominent subject: banners, placards, protest signs, posters, storefront signage, menu boards, chalkboards, newspapers, book covers, record sleeves, and product labels. Naming one as a focal element ("hand-painted banners catching hard light") makes the generator fill it with large garbled pseudo-writing. Such props may appear only distant, defocused, or angled away — and the subject must say so ("banners soft and unreadable in the background"). If the concept needs the prop's message, the message is real HTML typography near the image, never inside it.
- The same ban covers APP INTERFACES, DASHBOARDS, and DEVICE SCREENS as the dominant subject. A phone/tablet/laptop screen "showing" an app, schedule, chart, or any readable UI is rendered text at scale: the model invents gibberish labels and fake numerals that instantly read as AI output. Never write a subject like "a phone screen filling the frame showing the app's day view with job cards and times". Sanctioned alternatives, in order of preference: (1) build the UI as a REAL HTML card — a styled group of headings, lists and captions — instead of an image; (2) a device photographed at an oblique angle, at over-shoulder distance, or with the screen softly out of focus, so the screen reads as a glowing surface with unreadable content, and say so in the subject ("screen content abstract and out of focus"); (3) drop the device and photograph the human context instead (the hands, the site, the workshop).
