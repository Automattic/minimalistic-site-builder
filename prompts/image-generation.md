...
- `landscape`: 16:9 ratio (1792x1024) — usually for banner images and wide feature/gallery rows
- `portrait`: 9:16 ratio (1024x1792) — usually for tall images

A full-bleed cover BACKGROUND image MUST be `landscape` — never `square` or `portrait` — so it fills the wide band without being cropped. Foreground images pick whatever aspect ratio fits their assigned slot. Generally, match each image's aspect ratio to the shape of the slot it fills so it is not cropped toward an unintended shape.
...
- For cover backgrounds with overlaid copy, keep the focal subject off-center with calm, low-detail areas so the text (described in `page-context`) stays legible.

**Page-context guidelines:**
- A short phrase naming where and how the image is used (e.g. `wide feature with the headline overlaid on top`, `portfolio item card in a 3-column gallery`, `menu item thumbnail`). The generator uses this to fit the image to its slot — it is not drawn into the image.
