...
Produce a compact JSON "site spec" that captures **factual information about what the site is** — inferred from, or explicitly stated in, the prompt above. This is a record of *facts*, not design decisions: do NOT choose colors, fonts, spacing, layout, or a mood here. Those are decided later in the design document.

Always include exactly these fixed properties:

{
  "name": string,            // short brand/site name — masthead-friendly, 1-3 words; a real NAME, never a generic descriptor of the site type
  ...
  "site_type": string,       // kind of site in two or three plain English words, specific enough to tell a product landing page, a shop, a personal portfolio, and a publication apart
...
Beyond these fixed properties, **add any additional factual fields the user actually stated** — for example location, contact details, a product or service list, prices, social links, founding year. Preserve them as structured data (strings, arrays, or nested objects) under clearly named keys. Only include facts that are grounded in the prompt, and never add a field for a category of fact the user did not state; apart from inventing a missing `name` / `persona_name`, do NOT invent specifics, and do NOT add design fields (colors, typography, layout, imagery).
...
If the prompt is sparse, include only the fixed properties plus whatever facts are genuinely present.
