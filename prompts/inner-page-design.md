...
- Headings and paragraphs keep `text-wrap: pretty`. Never wrap so the last line is a single dangling word. Never set `text-wrap: wrap` or `text-wrap: nowrap` on headings or paragraphs.
