...
Card anatomy — how ONE card `wp:group` is constructed. The DESIGN DIRECTION's **Card treatment** line picks a single construction for the whole site; when the direction carries no card-treatment line, default to `flush`. A "card" here means a group with its own box (backgroundColor, border, or radius) — apply the chosen construction to every such card that holds an image:

- `flush` (the modern default) — media bleeds to the card's edges; padding wraps only the text. The card `wp:group` gets the box styling (backgroundColor, border radius), `"className":"card-flush"` (the theme ships `overflow:hidden` for it so the card's radius clips the media), NO padding of its own, and `"style":{"spacing":{"blockGap":"0"}}` so the media meets the text panel. The image is the FIRST direct child of the card group, with its crop class (below) and NO radius of its own — the card group's radius does all the rounding. ALL text (heading, paragraph, list, buttons) lives in ONE inner `wp:group` that carries the card's padding.
- `framed` — the inset construction: the card group carries padding on ALL sides and the media sits inside that padding (polaroid, print, scrapbook moods). Corners must be CONCENTRIC: with card radius R and padding P, the media's own radius is max(R − P, 2px) — card radius 24px + padding 32px → media radius 2px; card radius 16px + padding 8px → media radius 8px. NEVER give the media the same radius as its card; equal radii with padding between them read visually wrong.
- `overlap` — flush media with the text panel riding up over the image's bottom edge. Build exactly like `flush` (card group with `"className":"card-flush"`, image first, no card padding), but the inner text group additionally gets `"className":"overlap-up"`, its OWN backgroundColor, its own padding, and small side margins (`"style":{"spacing":{"margin":{"left":"1rem","right":"1rem"}}}`) so the image shows around the panel.
- `borderless` — no card box at all: the card group gets NO background, border, radius, or padding; the cropped image sits above a plain text stack and whitespace alone separates cards. Here (and only here) the image may carry a small radius of its own.
...
1. `equal-grid` — uniform card row, for flat hierarchies (pricing tiers, a trio of equally weighted features):
   ...
   - For a bottom-aligned CTA, wrap it in `wp:buttons` with `"className":"cta-bottom"`.
     (The supporting `.equal-cards` / `.cta-bottom` / `.card-media*` / `.card-flush` CSS already ships in the theme's style.css — just use these class hooks.)
