...
- Wrap the whole section in a single top-level <!-- wp:group --> with a constrained or full layout, so it drops cleanly into the page in order. Give that group the section's anchor — `"anchor":"{{section_slug}}"` in its JSON attributes and the matching `id="{{section_slug}}"` on its opening tag — so navigation and buttons can deep-link it (href="#{{section_slug}}" within the page, href="/#{{section_slug}}" from elsewhere).
