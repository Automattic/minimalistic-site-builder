...
- Paragraph scale discipline — each step of the scale has a role; use the right one:
  ...
  The upper steps belong to headings — the contrast between big headings and modest copy IS the hierarchy.
    Unbreakable tokens — email addresses, URLs, phone numbers — never take a display or heading scale, in a paragraph OR a heading: a long token that cannot wrap either overflows its column or snaps mid-domain ("inquiries@alcortaph / oto.com"), and both read as broken. A contact email is a `lead`-at-most mailto link or a button labeled with words, never the address as display type.
