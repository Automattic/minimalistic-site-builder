...
- Keep the accent color RARE: buttons/CTAs, plus the DESIGN DIRECTION's `signature_device` motif when the direction explicitly commits accent to it — then apply that motif consistently, in the direction's own terms, not in some sections only. Never use accent for body text, large-area backgrounds, or any motif the direction didn't name.
...
- Write real, specific copy in the brand voice grounded in the site spec — never lorem ipsum. When the DESIGN DIRECTION commits a **Headline register**, every heading and standout line speaks in that register — it is the site's committed voice, not a suggestion. The register's example fragment is REFERENCE ONLY (it is deliberately from another industry): never use it, translate it, or lightly adapt it as copy — you author every actual line fresh, in that voice, about this site. And speak it about THIS section's own content: never reuse or rephrase another section's heading.
...
- The H1 carries an IDEA, written in the DESIGN DIRECTION's committed **Headline register**. It must NOT be the bare brand name alone — the header's wordmark already names the site, and a brand-name masthead says nothing; put the name in an eyebrow or the supporting line if it needs to appear. Avoid the stock "verb the noun. Not the pain-point." formula unless the committed register is genuinely imperative.
- HARD H1 BUDGET: at most 8 words (~60 characters) and at most 2 sentences. The register commits the VOICE, never the length — a deadpan register is "Three crews. One schedule.", not a paragraph read aloud. At masthead scale every extra word costs a wrapped line; the great heroes in any register are terse. Say the idea's sharpest fragment in the H1 and move the rest of the sentence into the lead line.
- When the H1 sits inside a `wp:column` at 50% width or less, tighten further: at most 4-5 short words, no word longer than ~9 characters (long words break mid-word in a narrow column at display scale) — or drop the heading to `"fontSize":"section-title"` instead of `display` and say so with the composition. Never pin lines with `<br>` in a narrow column.
- `display` size belongs where the headline has ROOM: laid directly on the open image, or on a measure of 55%+ of the band. Inside a boxed panel (a background-colored group) or a column at 50% or less, use `"fontSize":"section-title"` instead — panels are pixel-capped while the display size grows with the viewport, so on wide screens a display H1 in a panel wraps one word per line.
- A PORTRAIT foreground image in the hero must be height-capped or it pushes everything beside/below it out of the first viewport: give its wp:image `"aspectRatio":"3/4","scale":"cover"` (serialized as `style="aspect-ratio:3/4;object-fit:cover"` on the img), or pick a `landscape`/`square` asset instead. Never bottom-align hero text against an uncropped portrait image (a deterministic pass stamps the 3/4 crop on uncropped portrait hero images).
- HERO TEXT BUDGET — the default hero is THREE text elements: the H1, ONE lead line, the buttons. An eyebrow (`caption`) or one microcopy line (`caption`) may join ONLY when a fact genuinely must sit above the fold — and the ceiling is 3 paragraph blocks in the ENTIRE hero section, everything counted. The hero image gets NO caption, NO dish/product label, NO name-plus-description cluster — a labeled image is a card, and cards live in later sections. Never state the same fact twice (a date in a banner AND in microcopy). If the brief hands you more facts than the budget holds, the extras belong to the next section, not the hero. A deterministic pass counts the hero's paragraphs and flags overruns.
- FIRST VIEWPORT: the H1 and the primary CTA must be visible without scrolling. Put them INSIDE the opening cover (or beside a contained image) — never below a tall opening image. If the committed hero composition seems to ask for a headline under the image, keep that image band at "minHeight":50-55vh at most so the masthead still lands above the fold (a deterministic pass shortens taller headline-less covers to 55vh).
...
- Use theme.json gradient and shadow presets (`"gradient":"<slug>"` on cover/group backgrounds; the shadow presets for depth), color blocks, typographic scale, and spacing rhythm via the `"style":{"spacing":{...}}` attribute on group/heading wrappers.
- **Eyebrows are rationed.** A small uppercase letterspaced label (kicker) above a heading is NOT default heading dressing — an eyebrow over every section heading is template filler exactly like a hairline under every heading, and it flattens the page's rhythm. Default is heading-first: the heading, the band background, and type scale already carry the hierarchy. Add an eyebrow ONLY where it does real work: it states genuine metadata or way-finding the heading itself doesn't (a date, a venue, a category on an index), or the DESIGN DIRECTION's `signature_device` explicitly commits to a labeling treatment — then execute exactly that treatment where the direction says, not everywhere. Never prefix sections or cards with sequence numbers ("01", "02"…) unless the signature device explicitly commits to numbering.
- **Lines are rationed.** `wp:separator` blocks and decorative borders (`"style":{"border":{...}}`) are the most overused decoration — a page stitched together with hairlines under every heading and boxes around every card reads as template filler, not design. Delineate content with whitespace, type scale, and the assigned band background first; those already carry the hierarchy. A separator or border is justified ONLY where it does real work: the `list-thumb` index/menu recipe, a genuinely tabular row, or the DESIGN DIRECTION's `signature_device` when it explicitly commits to rules — and even then at a few deliberate moments, applied consistently. Never end or begin a section with a separator (section seams belong to the page-level rhythm pass), and never use one just to "close" a heading or split two paragraphs.
