...
- `framed` — the inset construction: the card group gets `"className":"card-style--framed"`, carries padding on ALL sides, and the media sits inside that padding (polaroid, print, scrapbook moods). Corners must be CONCENTRIC: with card radius R and padding P, the media's own radius is max(R − P, 2px) — card radius 24px + padding 32px → media radius 2px; card radius 16px + padding 8px → media radius 8px. NEVER give the media the same radius as its card; equal radii with padding between them read visually wrong.
