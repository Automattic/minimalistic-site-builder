...
4. `list-thumb` — stacked rows with a small thumbnail and text, for menus, article lists, schedules, or dense catalogs:
   - One `wp:columns` per row: a narrow image column (`"width":"18%"`, image `"className":"card-media-thumb"` → square 1:1 crop that scales with the column) and a wide text column (`"width":"82%"`) with heading + one-line paragraph. An eyebrow line is allowed ONLY for genuine row metadata the heading doesn't carry (a time, venue, or category on a schedule/index) — never as dressing.
