...
- HARD FACTS: dates, times, prices, street addresses, phone numbers, and capacities come only from SITE SPEC, verbatim. Every section is authored independently, so an invented specific WILL contradict a sibling section (two sections of one page have shipped two different festival weekends). When the spec lacks the specific value, write copy that does not need it — "two nights this autumn", "from opening until late" — never a concrete number or date of your own.
...
- Use theme.json gradient and shadow presets (`"gradient":"<slug>"` on cover/group backgrounds; the shadow presets for depth), color blocks, typographic scale, and spacing rhythm via the `"style":{"spacing":{...}}` attribute on group/heading wrappers. Shadow presets belong on media, cards, and cover surfaces ONLY — never on a heading or paragraph block: a box-shadow on a text block traces the block's bounding box and renders as stray bars around the copy, and a deterministic build step strips it.
...
1. `equal-grid` — uniform card row, for flat hierarchies (pricing tiers, a trio of equally weighted features):
   ...
   - Any card image: add `"className":"card-media"` to the wp:image and copy that hook alone to its wrapper (`<figure class="card-media">`) — the theme's style.css crops it to a uniform 3:2 landscape ratio that scales with the column width. NEVER write the cropping as inline CSS.
...
3. `editorial-row` — one dominant card plus supporting cards, for curated/selected-work sections:
   - `wp:columns` with mixed widths that sum to 100% (e.g. 50/25/25 or 60/40); the dominant column gets the bigger image (`"className":"card-media-tall"` → tall 4:5 portrait crop) and a larger heading; supporting cards stay on `"className":"card-media"` (3:2 landscape).
4. `list-thumb` — stacked rows with a small thumbnail and text, for menus, article lists, schedules, or dense catalogs:
   - One `wp:columns` per row with `"isStackedOnMobile":false`: a narrow image column (`"width":"18%"`, image `"className":"card-media-thumb"` → square 1:1 crop that scales with the column) and a wide text column (`"width":"82%"`) with heading + one-line paragraph. `isStackedOnMobile:false` is MANDATORY for BOTH flush and framed rows — without it, Core turns the small thumbnail into a full-width image above the text at 781px. Never put an eyebrow or kicker line above the row heading — the ban on kickers above headings applies here too.
