...
Card anatomy — how ONE card `wp:group` is constructed. The explicit ASSIGNED CARD STYLE machine contract below picks a single construction for the whole site and overrides absent or conflicting prose in the DESIGN DIRECTION. Every outer image-card group MUST carry exactly ONE universal treatment marker matching that assigned construction: `card-style--flush`, `card-style--framed`, `card-style--overlap`, or `card-style--borderless`. Put the marker on the outer card group only; `flush` and `overlap` also carry the `card-flush` behavior hook. An "image card" here is an outer repeated-content group pairing one primary image with its related text and actions. The assigned treatment determines whether that group has a visual box; apply the chosen construction to every such image card:
