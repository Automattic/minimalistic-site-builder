...
- **Alignment discipline:** center display type only — headings, kickers, pullquotes, one-line leads. Any paragraph that wraps to several lines is set left-aligned, even inside a centered composition, on a cover, or under a centered heading: centering the COLUMN is composition, centering a paragraph's rag makes it hard to read. When copy sits in a centered column, the paragraph stays `"align":"left"` inside it.
