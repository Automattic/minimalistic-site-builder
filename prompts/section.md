...
- NEVER include site chrome in the section: no wordmark, no site-title lockup, no navigation or menu links — even if the DESIGN DIRECTION or Notes mention them. The header is a separate part; skip chrome instructions such as "wordmark top-left" or "nav reduced to one link".
...
- Paragraph scale discipline — each step of the scale has a role; use the right one:
    ...
    `caption` = short metadata, labels, eyebrows, gallery captions, and folio lines. Use `body` for a paragraph longer than 120 characters.
  ...
  The upper steps belong to headings — the contrast between big headings and modest copy IS the hierarchy.
    `display` is the FRONT PAGE hero's masthead; nothing else on the site uses it. Never author it here. An inner page's opening section owns that page's one h1, but still sits below the front headline: give that h1 `"fontSize":"section-title"` explicitly — an h1 with no `"fontSize"` inherits the masthead step from the theme. A deterministic pass lowers any h1 delivered at that step.
...
- **Heading treatment is global:** the DESIGN DIRECTION's **Type treatment** owns heading `textTransform` and `letterSpacing`. Do not set either value in a `wp:heading` block's typography attributes. For `sentence` and `tight`, author sentence-case text (capitalize only the opening word and proper nouns), because their safe CSS transform is `none`. Preserve the intended words for every treatment and let the committed site-wide style render their final case and tracking.
...
- **Eyebrows are banned.** A kicker or eyebrow above a heading is a ban, not a default: no brief earns it back. The only exceptions are the committed `section-badge` (a pill the build paints) and the committed `side-label` (a label in the leading column of a split), see the Section label rule below; an unmarked or uncommitted label is still an eyebrow. Do not emit a small uppercase letterspaced label, caption-scale line, or minor heading above a heading. Orientation copy (date, venue, category) belongs in the heading itself, a standfirst below it, or genuine body metadata that is not stacked on the heading.
