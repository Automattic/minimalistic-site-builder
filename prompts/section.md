...
- FORMS: never emit raw `<form>`, `<input>`, `<textarea>`, or `<select>` HTML. When the brief asks for a contact, booking, RSVP, or signup form, build a working Jetpack Form with `wp:jetpack/contact-form` and self-closing Jetpack field blocks such as `wp:jetpack/field-name`, `wp:jetpack/field-email`, `wp:jetpack/field-telephone`, `wp:jetpack/field-text`, `wp:jetpack/field-textarea`, `wp:jetpack/field-select`, `wp:jetpack/field-radio`, `wp:jetpack/field-checkbox`, or `wp:jetpack/field-consent`. Use only the fields the brief needs, ground labels and options in the spec, and never invent a recipient address.
- When the spec gives no exact recipient, omit the form's `to` attribute so Jetpack sends submissions to the site's admin email. When it gives no success message, keep Jetpack's standard success response. For an ordinary contact form, make name and email required; make other fields required only when the request makes them essential.
- A Jetpack Form's submit control MUST be a `wp:button` (`core/button`) with `"tagName":"button"`, `"type":"submit"`, and `"className":"form-button-submit is-submit"`; its saved HTML MUST contain `<button type="submit" class="wp-block-button__link wp-element-button">`. NEVER use `wp:jetpack/button` for a form submit control: without an explicit element it renders as a link to `#` and silently fails to submit. Canonical shape:
  `<!-- wp:jetpack/contact-form --><div class="wp-block-jetpack-contact-form"><!-- wp:jetpack/field-name {"label":"Name","required":true} /--><!-- wp:jetpack/field-email {"label":"Email","required":true} /--><!-- wp:jetpack/field-textarea {"label":"Message"} /--><!-- wp:button {"tagName":"button","type":"submit","className":"form-button-submit is-submit"} --><div class="wp-block-button form-button-submit is-submit"><button type="submit" class="wp-block-button__link wp-element-button">Send</button></div><!-- /wp:button --></div><!-- /wp:jetpack/contact-form -->`
- Use valid core block markup (group, cover, columns/column, heading, paragraph, buttons/button, image, gallery, media-text, quote, pullquote, list, separator, spacer; query/post-template only if useful), plus the Jetpack Forms blocks above when the section calls for a form.
...
- **NO decorative HTML comments** — never write `<!-- Services -->` and the like. Only `<!-- wp:... -->` block comments are allowed.
