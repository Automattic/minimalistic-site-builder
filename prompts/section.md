...
- **Column balance:** when a row pairs a media column with a text column, the text must plausibly fill at least half the media's rendered height — two caption lines beside a tall photo strand a blank quadrant of background that reads as a layout mistake. When the text is that short, put it UNDER the media, shrink the media to a modest inset, or fold the lines into a neighboring column instead of holding a column open for them.
