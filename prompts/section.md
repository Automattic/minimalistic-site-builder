...
- **Eyebrows are rationed.** A small uppercase letterspaced label (kicker) above a heading is NOT default heading dressing — an eyebrow over every section heading is template filler exactly like a hairline under every heading, and it flattens the page's rhythm. Default is heading-first: the heading, the band background, and type scale already carry the hierarchy. Add an eyebrow ONLY where it does real work: it states genuine metadata or way-finding the heading itself doesn't (a date, a venue, a category on an index). Never prefix sections or cards with sequence numbers ("01", "02"…).
