...
- NO EYEBROW — no label precedes the H1. The committed section-badge and side-label exceptions never apply to a hero or page opening. Put useful orientation copy in the standfirst.
- TEXT BUDGET — the copy region holds one H1, at most one supporting paragraph, and the planned action. The standfirst is one sentence, about two set lines and no more than roughly 180 characters. Do not fold overflow into the standfirst. Preserve a clear first-viewport hierarchy.
- HEADLINE EMPHASIS — when the DESIGN DIRECTION carries a Heading emphasis fact, mark one clause with `<span class="emph">…</span>` as directed. Do not style the span or mark supporting copy. Without that fact, omit it.
- HEADLINE COPY — use a concise proposition. Meaningful punctuation and ranges are allowed; avoid a repeated label-plus-slogan formula.
...
- Use valid core blocks only: group, cover, columns/column, heading, paragraph, buttons/button, image, media-text, spacer, separator. Use only blocks the assigned recipe needs. A `wp:separator` may organize the composition when it serves the concept; do not add one automatically.
...
- An invented factual claim includes a count of stock, customers or members, an operational policy such as vetting or quality checks, a guarantee, and any term of business the spec does not state. Never invent these claims.

Hero motion (optional; the DESIGN DIRECTION's Motion value is authoritative):
- `word-reveal` may mark the H1 once under an animated profile. Under calm or dramatic, prefer it for a display-scale headline of twelve words or fewer unless the composition calls for a whole-block entrance. Never combine it with another motion class on that heading.
