...
SCANNED USAGE (what the generated markup and theme.json actually reference, per family — your request MUST cover ALL of this):
{{usage}}
...
- The css2 URL must request every scanned weight for its own family (`wght@` axis, or `ital,wght@` tuples in ascending order for families where italics are used). You MAY add further weights or axes the design direction calls for.
- The only URLs in the file should be on fonts.googleapis.com / fonts.gstatic.com.
