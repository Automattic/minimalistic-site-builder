  ...
  "sections": [
    {
      ...
      "layout_archetype": "one of: full-bleed-cover, asymmetric-split, centered-stack, offset-grid, equal-card-grid, list-with-thumbnails, bento-grid, faq-split, cta-panel, pricing-tiers, stat-ledger, feature-row-hairlines, zigzag-steps, statement-lines, project-grid-2x2, logo-strip",
...
- centered-stack — ONLY a super-simple, single-message section: one heading, one short paragraph (two or three lines), optionally one action or one small supporting image. A brief standalone statement or invitation can qualify; the theme centers every element in the band. Never use it for a story with several paragraphs, a process, features, menu, schedule, directory, repeated items, or a form. It is an occasional pause, never the default for text-led content, a compact opening, or a closing. Assign item_pattern: null. If the content needs more structure, choose the composition that serves that structure; do not trim required content to make it fit.
...
- list-with-thumbnails — stacked rows for a menu, schedule, article index, or dense catalog where each thumbnail identifies its item. Use one heading and at most one short introduction paragraph. Use equal-card-grid for a short set of address, hours, and contact details. Do not add images only to fill a thumbnail slot.
...
- The DESIGN DIRECTION's **Rhythm** governs every archetype and background choice below — it is the site-level intent these per-section picks express, so read it FIRST and plan the page against it rather than assigning each section in isolation. `stacked` means an orderly vertical sequence of content-led compositions, not a page of centered-stack sections: a process still needs steps, a feature set still needs a row or grid, and a story still needs its own editorial structure; `alternating` gives consecutive sections visibly different compositions (archetypes, not backgrounds — the background rule below still governs those); `offset` favours asymmetric-split, and offset-grid where this site is eligible for it, while avoiding centered-stack; `interrupted` places at least one "image" or "contrast" full-bleed band per page against an otherwise steady stack; `banded` spends the page's allowance of contrast/tinted bands rather than carrying the page on archetype changes; `gallery` favours grid and image-led bands over text-led ones. One archetype used for most of a page is a failed plan under every rhythm, including `stacked`. Rhythm never overrides an archetype’s content requirements.
...
{{footer_surface_rule}}
- Plan width rhythm within the compositions the content needs: constrained lead copy can sit inside a wider split, row, or grid, while image-led bands can run wide or full. Never choose centered-stack merely to obtain a narrow beat or satisfy variety. A centered text_placement moves the copy column; it does not require the centered-stack archetype.
