...
- offset-grid — a staggered grid whose items deliberately don't line up in neat rows. Use ONLY when the DESIGN DIRECTION's rhythm is `offset` or `gallery`. Under every other rhythm, pick a level row (equal-card-grid, asymmetric-split, or bento-grid) instead of offsetting sibling tops.
- equal-card-grid — a row of equal-height cards. Prefer cards for a short set of practical details, such as address, hours, and phone.
- cta-panel — the closing invitation as ONE contained, rounded panel on the page ground (the build rounds it from the committed shape): one heading, one line, exactly one action, optionally one image beside the copy. Plan it for the LAST section whenever that section carries a `primary_action` and the page does not already end on a full-bleed cover. Its `background` is `base` or `tinted`: the panel, not the band, carries the contrast.
...
- logo-strip — one quiet row of four to eight client, partner or press names set as words in the heading face, small and muted, with an optional one-line lead above it; for a trusted-by, as-seen-in or partners strip right after the hero or before pricing. Plan no images and no item_pattern for it: the names are words, never logo pictures.
...
- The DESIGN DIRECTION's **Rhythm** governs every archetype and background choice below — it is the site-level intent these per-section picks express, so read it FIRST and plan the page against it rather than assigning each section in isolation. `stacked` means an orderly vertical sequence of content-led compositions, not a page of one repeated composition: a process still needs steps, a feature set still needs a row or grid, and a story still needs its own editorial structure; `alternating` gives consecutive sections visibly different compositions (archetypes, not backgrounds — the background rule below still governs those); `offset` favours asymmetric-split, and offset-grid where this site is eligible for it; `interrupted` places at least one "image" or "contrast" full-bleed band per page against an otherwise steady stack; `banded` spends the page's allowance of contrast/tinted bands rather than carrying the page on archetype changes; `gallery` favours grid and image-led bands over text-led ones. One archetype used for most of a page is a failed plan under every rhythm, including `stacked`. Rhythm never overrides an archetype’s content requirements.
...
- Plan the background rhythm deliberately: `base` is the resting surface, with 1-2 purposeful `tinted`, `contrast`, or `image` beats placed for pacing where THIS page's content genuinely changes register. On every page of {{min_banded_sections}} or more sections, use AT MOST {{max_non_base_sections}} non-base backgrounds across the whole page — never alternating stripes. More content does not earn more colors, and even a `multicolor` palette does not mean every section needs a different surface. The build demotes excess bands to `base`, so spend this budget on the two transitions that matter most. If the build later moves your closing section off the footer's surface, that one correction sits outside this budget.
- "Mostly base" is not "all base". A page of {{min_banded_sections}} or more sections MUST carry at least one "contrast", "tinted" or "image" band — a plan where every section sits on the page background is rejected. This matters MORE on longer pages, not less: a 7-section page with no band is one unbroken scroll of the same colour, which is exactly where a reader loses their place.
{{footer_surface_rule}}
- Plan width rhythm within the compositions the content needs: constrained lead copy can sit inside a wider split, row, or grid, while image-led bands can run wide or full. Never choose an archetype merely to obtain a narrow beat or satisfy variety. A centered text_placement moves the copy column; it does not change the archetype.
...
- A **Surface** fact permits a texture; it does not require one. Assign its `surface--<name>` class in `content_notes` to at most one suitable section per page. State how that section's content supports the texture's design reason. Leave every section plain when none benefits. Keep the hero, header, footer, tables, forms, navigation, and sections with a decorative device plain. Never repeat a texture across the page.
