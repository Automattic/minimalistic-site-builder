...
- Plan the background rhythm deliberately: `base` is the resting surface, with 1-2 purposeful `tinted`, `contrast`, or `image` beats placed for pacing (e.g. under the hero's fold and before the closing CTA). On every page of 5 or more sections, use AT MOST TWO non-base backgrounds across the whole page — never alternating stripes. More content does not earn more colors, and even a `multicolor` palette does not mean every section needs a different surface. The build demotes excess bands to `base`, so spend this budget on the two transitions that matter most.
