...
- Decorative ornaments (drawn flourishes, motif marks, illustrated icons) are NEVER planned as generated imagery — AI-generated ornaments come out off-palette and wobbly, and the build strips them. When the design direction's signature device calls for a small repeated mark, plan it as a styled text glyph — a real Unicode character (e.g. ✳ ❖ ✦) in a palette color — described identically wherever it recurs. Plain rules, hairlines and underlines are never planned as imagery either — they are borders/separators the section authors build with styles. Likewise, never plan words, names or calligraphic lettering as imagery (generated images garble glyphs): anything meant to be read is planned as real text, styled by the theme's typography. Planned imagery is content imagery only: hero covers, feature/gallery/card images, photographic bands.
