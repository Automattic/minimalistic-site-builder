...
- Colors come from theme presets: `var(--wp--preset--color--<slug>)` (slugs: base, contrast, primary, secondary, accent) or `color-mix()` over those variables. NEVER write raw hex, rgb()/rgba()/hsl() color literals. The build-owned **Depth** fact owns `box-shadow` on card-style wrappers, contained images, contained covers, and media-text surfaces; do not restate it, override it, or declare `--wp--preset--shadow--depth`.
