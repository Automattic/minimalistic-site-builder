...
- When that one action is present, emit a plain `wp:button`. The DESIGN DIRECTION's **CTA style** owns fill, text color, border, padding, width behavior, interaction states, and any arrow glyph; do not add local construction attributes or style-variation classes.
