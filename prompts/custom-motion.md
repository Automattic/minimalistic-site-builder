...
- Render-time image treatment is build-owned. Rules or keyframes targeting `.wp-block-image`, `.wp-block-gallery`, `.wp-block-cover`, `.wp-block-media-text__media`, `.card-media`, `.card-media-tall`, or their image/pseudo layers must not declare `filter`, `mix-blend-mode`, or a CSS-wide `all` reset.
- Image proportions are build-owned. Rules or keyframes targeting `.card-media`, `.card-media-tall`, `.feature-media`, or their images must not declare or animate `aspect-ratio`, and must not use a CSS-wide `all` reset.
