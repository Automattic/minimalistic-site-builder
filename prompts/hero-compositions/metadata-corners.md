...
Build the opener as one full-bleed portrait cover with the copy a start-aligned
stack centered in the frame and two or three small facts about the studio
riding the top corners, the way an editorial spread labels its margins. The picture owns
...
- Structure: the root's one direct visual child is a `wp:cover` at
  ...
  one planned button, all start-aligned (the build centers the block in the
  frame; the lines stay flush at their start edge); then, LAST, ONE `wp:group` with
...
- Surface/width: the planned `image` surface with the root and cover at
  ...
  fallback surface.
- Mobile: the same cover, the facts in one row at the top, the copy centered
  in a shorter stage.
- Objective failure: no direct cover, more than one image, no
  ...
  fact inside or before the copy group, or copy pinned to a corner.
