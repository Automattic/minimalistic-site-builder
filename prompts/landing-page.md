...
- Where imagery belongs (hero covers, feature/gallery/card images), emit a generatable AI image placeholder using ONLY the native src and alt attributes — no custom data attributes:
    ...
    alt — the image-generation spec in this EXACT format: "AI_IMAGE: <description> | <style> | <aspect-ratio>"
      <description>: 1-3 specific sentences (composition, colors, mood, subject) — this is the generation prompt. Start with where and how the image is used on the page, since you are placing it and know its role: name the section and its purpose (e.g. "Full-bleed hero background for the homepage, with the headline overlaid on top —" or "Portfolio gallery card for the 'Plaza de Mayo' essay —"), then describe the image. For cover/hero backgrounds, state that text is overlaid and ask for the focal subject off-center with calm, low-detail areas so the overlaid text stays legible. Make sibling images in the same section describe their distinct subject so they don't read alike.
    ...
    Give every image a UNIQUE filename. Example:
      <!-- wp:image {"sizeSlug":"large"} --><figure class="wp-block-image size-large"><img src="theme:./assets/loaf-sourdough.jpg" alt="AI_IMAGE: Menu card for the bakery's signature loaves section. A rustic sourdough loaf with a crackled golden crust on a floured wooden board, warm side light | photorealistic | square"/></figure><!-- /wp:image -->
