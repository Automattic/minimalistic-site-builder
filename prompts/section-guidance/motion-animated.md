...
- `sticky-stack` — on ONE `wp:columns` or card-grid `wp:group` with two to six cards: pins cards under the header successively, each slightly lower. The kit makes a single column (services, process, testimonials); never use on a grid that must stay side by side. One stack per page; no entrance-budget cost; static under reduced motion.
- `count-up` — on a heading or paragraph containing ONLY a figure ("120+", "98%", "$4.2M", "1,200"): counts from zero on viewport entry, preserving prefix, separators, decimals and suffix. Put each metric on its own block; no entrance-budget cost. Never on a sentence, date, or number that is not the point.
...
- `marquee` — AMBIENT: on ONE `wp:paragraph` with a short phrase (two to eight words, written ONCE). The kit repeats it, loops sideways, pauses on hover, and stays still under reduced motion. Never on a heading; one per page. Authored repetition ("Studio — Studio — Studio") is removed as a defect.

Profile choreography (follow the DESIGN DIRECTION motion note when more specific):
{{profile_choreography}}
All kit movement is vertical: an entrance rises, unmasks, sharpens, or settles in place — nothing moves sideways, except the one `marquee` line the kit scrolls. Vary the entrances down the page: neighboring sections should not repeat the same `reveal-*` class when a different one fits their content.
...
- The `marquee` has its own budget: ONE loop per page, on a paragraph, and it does not spend the ambient slot. The three image-and-surface AMBIENT classes are signature effects: at most ONE ambient effect on the WHOLE page, and only if this section is the page's focal moment. Look at the COMPOSITION block's neighbors — mid-page support sections get no ambient motion.
