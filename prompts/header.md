...
3. **minimal-overlay** — the header starts over a full-bleed hero: top-level wp:group with NO "backgroundColor" at all and a light "textColor" selected by the deterministic contract. A trusted shell supplies the reviewed translucent contrast veil and later opaque surface; do not imitate either in generated markup. No buttons, no clutter — a small site title and a quiet nav at most. Do NOT add the legacy `header-overlay` class or any positioning: the trusted outer theme shell owns the overlay and sticky/scrolled states, and a deterministic pass adds the reviewed visual-state classes. Do not add `<style>` tags. Only ever assigned when the build has verified the hero is an image-led, full-bleed cover it can float over AND every page's opening section can support the same foreground — never when the DESIGN DIRECTION's **Canvas** is `framed`: the overlay spans the full viewport while a framed hero keeps a mat of page background around it, so the header would hang over the mat instead of the image.
...
- Scroll behavior is the deterministic assignment above, not an authored block style. NEVER add `style.position`, inline `position`, legacy `header-overlay`, behavior classes, CSS, or JavaScript. The trusted outer theme shell owns positioning and the deterministic backstop chooses safe top/scrolled palette surfaces and foreground color from the real theme palette.
