...
**Subject anchor.** Take the swap test literally: if this site's subject were swapped for an unrelated one, which of your choices would break? At least one must. Name the one thing in the subject's OWN physical world — a material, an object, a surface, a light, a color that exists on the thing this site is about — that a palette role is taken from, and say which role carries it: `base`, `band`, `contrast`, `primary`, `secondary`, or `accent`. A direction whose every hex could belong to any business in the category has no anchor; a direction whose `accent` is the color of the product's cut flesh, or whose `band` is its packing crate, does. Ground it in the subject, not in the category's atmosphere.

**Tension.** Name the one deliberate contrast the direction is built on: two things from different worlds held against each other, both expressible in the vocabulary below — a spatial register against a color, a letterform tradition against a photographic grade, a scale of type against a scale of image. Precision against ripeness; a ledger's discipline against a harvest's loudness; a Didone's hairlines against a factory floor. A direction with no tension resolves into the category's default by construction, because nothing in it argues. Write it as one sentence that names both halves; downstream authors receive it as a fact and are told that a band holds both halves rather than resolving into one.
...
- `hero_blueprint`: fill the bounded front-page-only object using the composition choices below. Choose the recipe and permitted spatial values to serve the concept, unless the caller explicitly assigns a recipe. Do not mention that recipe, its topology, its media placement, or any hero-specific spatial arrangement in `description`; downstream general prompts receive the narrative but must not learn the front-page structure through prose.
