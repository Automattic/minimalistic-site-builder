...
- `motion_note`: a LIST of motion-kit class names the profile should favor, or `[]` when the profile alone says enough. These are class names, not art direction: write `["stagger-children"]`, never `"cards rise one by one"`. Pick from `reveal`, `reveal-up`, `reveal-fade`, `reveal-scale`, `stagger-children` (entrance — at most one), `ken-burns`, `gradient-shift`, `ambient-drift` (ambient — at most one for the whole page), `hover-lift`, `hover-reveal` (hover — at most one). Never name `hero-entrance`; the hero owns it. Never pair `ambient-drift` with `hover-lift`, or `ken-burns` with `hover-reveal` — each pair fights over the same transform. A `minimal` profile may name only the two hover classes, and `none` may name nothing. Anything outside this vocabulary is dropped and recorded.
