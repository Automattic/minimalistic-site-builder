...
**Commit to visible, executable features.** Alongside the structured palette, typography, image, surface, shape, depth, composition and motion fields, the build supports per-design CSS decoration on generic group wrappers: `design-frame` for borders (including double, dashed and rounded outlines), and `design-motif` for empty normal-flow `::before`/`::after` shapes painted with gradients, borders, radius and clip-path. Linear, radial, conic and repeating gradients can express angular, curved, geometric or organic visual languages; these are capabilities, not a prescribed style. Motifs are bounded to 192px tall and their wrapper's width. They never overlay text or images. Name the hook, paint construction, palette role and intended placement in `style_signature`. Repetition and hero use are allowed when the concept warrants them; no decorative quota is a quality rule. Use no hooks when the concept is carried by type, imagery and space alone. The older `device` field is a separate optional one-band mark, not the ceiling on visual expression.

Do not promise assets this channel cannot create: intricate hand-drawn filigree, figurative illustrations, custom icons, crests, or lettering baked into images. Find an honest CSS-executable interpretation without erasing the user's requested visual language. A style label, matching colors and a related serif are not substitutes for its defining graphic structure. Safety, readable text, usable controls and valid responsive blocks remain binding; aesthetic restraint is not a universal requirement.

- `palette`: the six named hexes the theme will ship. `base` = page background, `band` = the large-area tinted-section surface, `contrast` = body text on base (strong contrast required), `primary` = main brand color, `secondary` = supporting small-text color, `accent` = for CTAs/interaction and small concept-led decorative details; never use `secondary` or `accent` for broad fills. Preserve a clear primary action hierarchy. `band` must stay in the same tint family and on the same light/dark side as `base`, with HSL lightness 6–14 points away (aim for 10). The build verifies this relation and deterministically replaces a missing or drifting band from the delivered base.
...
- `style_signature`: describe the observable features that make this interpretation recognizably belong to the requested style and subject, and where they appear. Name concrete typography/composition/image choices, plus `design-frame`/`design-motif` and their CSS construction when needed. Do not merely repeat the style label or force ornament into a minimal concept. Downstream authors receive this commitment; the build checks whether promised decorative hooks and paint actually reached the output, not whether the design is beautiful.
- `style_hooks`: list the decorative hooks this signature actually requires: `design-frame`, `design-motif`, both, or `[]` for no decoration. This explicit list drives delivery checks; a hook mentioned only to rule it out is not a commitment.
...
```json
{
  "direction": {
    ...
    "style_signature": "Observable features and their placement; include design-frame/design-motif and CSS construction only when this concept needs them.",
    "style_hooks": [],
    "type": {
      "heading": {
