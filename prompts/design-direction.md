...
- `heading_emphasis`: how ONE clause inside a heading is set apart from the rest — one of `"none"`, `"two-tone"`, `"italic-word"`, `"highlight"`. Executed literally by the build on a clause the section author marks: `two-tone` mutes the quieter clause of a heading to 70% ink beside the strong clause (the product-landing and portfolio voice: one heading, two tones, no second colour); `italic-word` swaps one to three words into italic, in the accent face when `type.accent` commits one (playful and editorial voices); `highlight` draws a translucent accent highlighter band behind one to three words (friendly product and fintech voices). `none` is a real commitment for concepts whose headings are one tone. Read it off the concept's voice: a direction that promises two-tone or italic headings in its narrative must commit the matching value here, or the page ships one-tone headings.
