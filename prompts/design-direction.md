...
- `shape`: the corner language for contained `core/image` media and buttons — `"sharp"`, `"soft"`, or `"round"`. One commitment for the whole site, executed literally by the build: `sharp` (the default) keeps contained images and buttons square; `soft` gives contained images a subtle radius and buttons a modest one; `round` gives contained images a decisive radius and buttons a pill shape. Full-bleed media always meets its edges square, whatever the shape. Generic card wrappers are outside this deterministic commitment, so do not promise their geometry from this field. Commit to `soft` or `round` ONLY when the concept's own visual world genuinely speaks in rounded geometry — never as a pleasant-looking hedge; rounding that nothing in the direction motivates reads as template styling, not a design choice, and a site whose corners disagree with its concept reads as a bug.
