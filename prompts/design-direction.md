...
**The narrative may only promise what these fields can execute.** It is not mood-board copy: it is handed verbatim to every downstream design and section prompt as the authoritative brief. A promise the vocabulary cannot express is not refused anywhere — it is simply never delivered, and the page ships plainer than the direction it was given. The whole vocabulary is: the six palette hexes and their `color_economy`, the type families and weights, `type_treatment`, `image_grade`, `image_crop`, `image_treatment`, `surface`, `shape`, `depth`, `card_style`, `item_pattern`, `cta_style`, `heading_emphasis`, `canvas`, `measure`, `rhythm`, `density`, `text_placement`, `header_chrome`, `motion`, and ONE `device` mark on at most one non-hero band. Everything the page can show is type, color, spacing, photographs, and those marks. The two prose commitments, `subject_anchor` and `tension`, are written in that same vocabulary: they say where the hexes come from and what the composition argues, never what extra artwork to draw.
