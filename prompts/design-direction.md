...
- `device`: an optional one-band CSS ornament — one of `"none"`, `"hairline-rule"`, `"stamp"`. The page plan may place it on at most one non-hero band. Leave `"none"` unless the concept needs that one mark. There is no numeral device. Ordinary text may use numbering for a meaningful sequence or identifier; avoid numbers used only as repeated decoration. Twine, tape, and illustrated motifs are not devices.
