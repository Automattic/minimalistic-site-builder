...
- `device`: an optional one-band CSS ornament — one of `"none"`, `"hairline-rule"`, `"stamp"`. The page plan may place it on at most one non-hero band. Default to `"none"`; a hairline must divide a real table/index structure, not decorate a heading or section seam. There is no numeral device; the anti-tell numbering ban applies regardless of the concept. Twine, tape, and illustrated motifs are not devices.
- `canvas`: `"full-bleed"` or `"framed"` — choose how the composition meets the viewport. A framed opening is valid and uses a stacked header; the hero is not universally exempt from the frame. Explicitly assigned cover recipes retain their documented geometry.
...
- `hero_blueprint`: use the authoring instructions below to describe the opening's composition, rationale and mobile reading order. In ordinary blocks builds, authored means open composition, not another template. Keep hero-specific placement out of the site-wide description. Only an explicit caller recipe or capability limit constrains that arrangement.
...
```json
{
  "direction": {
    ...
    "hero_blueprint": {{hero_blueprint_shape}}
  }
}
```
