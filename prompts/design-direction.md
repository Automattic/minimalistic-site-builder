...
- `motion_note`: ONE short line that names kit classes the profile can ship (e.g. "cards rise one by one", "hero image breathe", "labels press on with overshoot"), or `""` when the profile alone says enough. A note the kit cannot express is stripped. "buttons press" is a class, not fake art direction.
