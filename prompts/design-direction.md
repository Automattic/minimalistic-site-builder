...
- **Image treatment**: Full-bleed photography that immerses? A contained frame that creates breathing room? Abstract shapes that suggest rather than show? A full-bleed background image should read wide/landscape; a framed or foreground image within the hero can be any shape that fits its slot. Drama comes from ONE strong image and how type meets it — not from stacking images; multi-image collages are off the table (see `hero_composition` below).
...
- `hero_composition`: the composition archetype the hero commits to, in one concrete sentence that pins down where the imagery sits, where the headline sits, and how the remaining space is used — composed for THIS concept, per the Hero Section guidance above. Describe only the hero section's OWN content — never the site header, wordmark, or navigation; those live in a separate header part rendered above the hero. Two hard constraints, because this sentence is executed literally by a builder working with simple blocks:
    1. **Simple anatomy**: at most ONE photograph plus at most ONE anchored content element (a text stack or a panel). Never commit collages — multiple overlapping images, framed-photo-with-corner-overlap arrangements, layered frame treatments — they cannot be built reliably and ship misaligned.
    2. **The first viewport earns its keep**: the headline and primary CTA must land INSIDE the opening viewport. Never commit a composition that pushes the headline below a tall opening image ("image fills the viewport, headline below it" is off the table — a deterministic pass will shorten such an image to force the headline up).
- `headline_register`: the voice the site's display copy speaks in — the seed's committed register (imperative command, deadpan spec-sheet, poetic fragment, customer's-own-voice, contrarian challenge, quiet understatement, documentary caption, …), refined into one concrete sentence. To demonstrate the voice, include ONE short example fragment (6 words or fewer) from a DIFFERENT, unrelated industry — never this site's topic (e.g. for a restaurant site, a deadpan register might be demonstrated with "Four strings. No amplifier." about a luthier). The example is reference-only: it shows the cadence at real masthead length while being unusable as this site's copy, because downstream writers author every actual headline fresh in this voice — a fragment in the brand's world WILL get pasted verbatim as the hero H1, which defeats the register. The register commits the voice, never the length or the words. Commit to ONE register and make it specific; if the seed carries no register, choose one that serves the concept. The H1 must carry an idea — never the bare brand name alone.
