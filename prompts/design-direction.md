...
- `motion`: how the page MOVES — one of `"calm"`, `"energetic"`, `"dramatic"`, `"minimal"`, `"none"`. The theme ships a fixed, hand-tuned motion kit (scroll reveals, staggered card entrances, one ambient signature effect); this profile sets its whole character: `calm` = slow, gentle, gallery-like reveals; `energetic` = quick, springy entrances; `dramatic` = long, cinematic sweeps; `minimal` = hover micro-interactions only, no scroll motion; `none` = completely static. Pick what serves the concept — a contemplative portfolio wants `calm`, a kids' brand `energetic`, a brutalist manifesto maybe `none`.
...
- `signature_device`: the ONE repeated visual motif that makes the direction recognizable on every section (e.g. "hairline rules with page folios", "oversized year numerals in the margins", "duotone image blocks with offset borders"). Prefer a device the theme can execute with tokens — borders, rules, type, color, spacing. If it includes a drawn/pictorial mark (a glyph, sprig, crest), it is ONE small ornament reused verbatim across the page, never a family of generated variants. The device must never depend on rendering words or letters as imagery (hand-lettered names, calligraphic lines, drawn mottos) — generated images garble glyphs, so lettering-flavored devices are expressed through real typography (font choice, size, style) instead.
