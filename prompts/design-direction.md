...
- `surface`: the page texture. Choose `"none"`, `"paper"`, `"concrete"`, `"film"`, `"fabric"`, `"noise"`, or `"dot-grid"`. The build supplies the CSS overlay. `noise` adds fine grain without lines. `dot-grid` adds one faint dot every 24px in the page ink. Choose `none` unless the concept needs a visible texture. Describe a texture in the narrative only when this field selects it.
