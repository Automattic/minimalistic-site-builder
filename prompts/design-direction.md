...
- `item_pattern`: how list-like content repeats across the site — one of `"card"`, `"rule-row"`, `"index"`, `"spec-table"`, `"tag-cluster"`. This is separate from `card_style`: `card` chooses bounded repeated units and then `card_style` determines any image-card anatomy; the other four deliberately escape the heading-plus-paragraph card grid. `rule-row` suits menus, schedules, directories, and price lists; `index` suits ordered programs, archives, stages, and workshop lineups; `spec-table` suits technical facts, product attributes, and amenity comparisons; `tag-cluster` suits short skills, genres, ingredients, and amenities. Commit one idiom that fits the concept's recurring catalog content. The page planner assigns it only to genuinely list-like sections, and each assigned section receives exactly that recipe.
...
- `density`: how tightly the page packs vertically — one of `"airy"`, `"measured"`, `"dense"`. Sections carry their own compact/standard/spacious assignment, and the build also scales the physical spacing scale from this commitment: the `lg`/`xl`/`xxl` section-padding ramp, the `xs`/`sm`/`md` component spacing, and the page's inline gutter all move together (the component swing is deliberately smaller). `airy` suits gallery and luxury concepts, `dense` suits catalogs, archives and editorial concepts with a lot to say.
...
```json
{
  "direction": {
    ...
    "item_pattern": "One bounded repeated-item idiom — card, rule-row, index, spec-table, or tag-cluster — chosen per the item_pattern field above.",
