...
- `step_numeral`: whether process steps carry their ordinal — one of `"none"`, `"chip"`, `"ghost"`. Decorative numbers stay banned; this is the one committed form and it applies to process, method, workflow and timeline sections only, where each step item may open with its digit. `chip` is a small round chip in the surface's own ink; `ghost` is a large translucent figure in the heading face above the step's heading. The build paints it and renumbers the steps in order. Commit `chip` or `ghost` when the brief asks for numbered steps or the concept is a product or studio page whose process reads as a sequence; leave `none` otherwise.
...
```json
{
  "direction": {
    ...
    "step_numeral": "One bounded step numeral — none, chip or ghost — chosen per the step_numeral field above.",
