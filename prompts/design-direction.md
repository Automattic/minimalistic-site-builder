...
- `shape`: the corner language for CONTAINED imagery and cards — `"sharp"`, `"soft"`, or `"round"`. One commitment for the whole site, executed literally by the build: `sharp` (the default) wires no radius anywhere; `soft` wires a subtle corner radius onto contained images; `round` wires a decisive one — and buttons follow the same language. Full-bleed media always meets its edges square, whatever the shape. Commit to `soft` or `round` ONLY when the concept's own visual world genuinely speaks in rounded geometry — never as a pleasant-looking hedge; rounding that nothing in the direction motivates reads as template styling, not a design choice, and a site whose corners disagree with its concept reads as a bug.
