...
- `band_geometry`: how a section band that paints its own surface meets the viewport — one of `"square"`, `"rounded"`. `square` runs edge to edge. `rounded` insets every contrast or band-coloured section band by one gutter and gives it the committed panel radius, so dark bands read as rounded plates on the page ground (the hero and image covers keep their edges); the build executes it, author nothing. Commit `rounded` when the brief names rounded bands or the concept is a contemporary portfolio or product page whose dark bands float on a light page; leave `square` for editorial, heritage and full-bleed concepts.
