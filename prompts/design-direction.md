...
- `signature_device`: the ONE repeated visual motif that makes the direction recognizable on every section. Derive it from THIS concept and the site's real-world visual culture — never from a stock menu of devices; if the same device would fit any other site unchanged, it is too generic. Prefer a device the theme can execute with tokens — type, color, spacing, and only sparingly borders or rules. Line-based devices (hairline rules, ruled boxes, separators) are the most overused choice and downstream steps tend to multiply them: commit to one ONLY when the concept genuinely calls for a print/editorial register, and define it as a few deliberate moments, never a divider under every heading or a frame around every block. The device must be executable WITHOUT generated imagery: never commit to a drawn or pictorial ornament (a sprig, crest, drawn emblem, illustrated icon, tick strip) as a generated image asset — AI-generated ornaments come out off-palette and geometrically wobbly, undermining the very direction they were meant to distinguish. The strongest devices need NO ornament character at all: they live in how type, color, spacing, or composition BEHAVE — and they are invented for this concept, not picked from a list; a device this prompt could enumerate would already be too generic. ANTI-HERDING: generated directions keep converging on the same few typographic defaults — a section numbering/index system ("01, 02, 03…" before every block), a hanging-indentation system, a repeated glyph mark, hairline rules. Treat these as ALREADY TAKEN: commit to one only when the site's actual subject matter makes it near-literal (a numbered tasting menu, an archive with real catalogue numbers) — and then this field must name the real-world source it quotes. Test your device before committing: if an unrelated site could adopt it unchanged, or it amounts to "every section opens with <the same label treatment>", it is a default, not a signature — design a different one. A small typographic glyph mark (a single existing Unicode character in a palette color) is a RARE exception, not a substitute ornament: commit to one only when THIS concept genuinely earns that specific character (a real cultural or topical grounding, not decoration for its own sake), and then define it as 1-2 deliberate moments on the page — never a mark before every heading or eyebrow, never a list bullet or metadata separator, never a divider stand-in. Asterisk/diamond glyphs (✳ ❖ ✦ and kin) sprinkled across a page are the new stock ornament — if the device amounts to "this glyph next to things, everywhere", replace it with a type/color/spacing device instead. The device must never depend on rendering words or letters as imagery (hand-lettered names, calligraphic lines, drawn mottos) — generated images garble glyphs, so lettering-flavored devices are expressed through real typography (font choice, size, style) instead.
