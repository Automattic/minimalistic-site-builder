...
All three are legitimate when the brief asks for them. Where the brief leaves the aesthetic free, landing in one means the self-check failed. The neutral monochrome product register — a near-white or near-black neutral ground, a tight light-weight grotesque, hairline rings, no accent hue — is a different look from #2, not a variant of it; it is legitimate for software, developer-tool, and technical briefs, and its executable form is `color_economy: monochrome`, `ground_tint: neutral`, `type_treatment: tight`, `depth: ring`. Two tests: if someone could guess your direction from the site's CATEGORY alone, rework it — and if they could guess it from the category plus the obvious avoidance ("not cream, so near-black"), rework it too.
...
- `motion_note`: a LIST of motion-kit class names the profile should favor, or `[]` when the profile alone says enough. These are class names, not art direction: write `["stagger-children"]`, never `"cards rise one by one"`. Pick from `reveal`, `reveal-up`, `reveal-fade`, `reveal-scale`, `reveal-blur`, `reveal-wipe`, `reveal-wipe-up`, `reveal-aperture`, `reveal-zoom`, `stagger-children`, `ken-burns`, `gradient-shift`, `ambient-drift` (ambient — at most one for the whole page), `hover-lift`, `hover-reveal` (hover — at most one). Treat this list as the site's motion palette: name the two or three entrance classes whose character belongs to the concept's visual world, the way the palette commits hexes — masked wipes (`reveal-wipe`, `reveal-wipe-up`) for print and editorial worlds, focus and zoom (`reveal-blur`, `reveal-zoom`, `reveal-aperture`) for photographic and cinematic worlds, plain fades and rises for quiet product worlds, `stagger-children` for rhythmic card-led worlds. Downstream sections choose from this palette first, so an empty list forfeits the site's motion identity to per-section defaults. Never name `hero-entrance`; the hero owns it. Per-block pairs that fight over the same transform (`ambient-drift`+`hover-lift`, `ken-burns`+`hover-reveal`) are motion-sanity's job, not this list. A `minimal` profile may name only the two hover classes, and `none` may name nothing. Anything outside this vocabulary is dropped and recorded.
