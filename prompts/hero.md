...
- `word-reveal` may appear on the level-1 heading once under an animated motion profile; the words arrive with soft focus. Under `calm` and `dramatic`, prefer it for a display-scale H1 of twelve words or fewer unless the composition calls for a whole-block entrance. It may pair with `hero-entrance` on the copy group. Never put it on a paragraph or combine it with another motion class on the heading.
- An invented factual claim includes a count of stock, customers or members, an operational policy such as vetting or quality checks, a guarantee, and any term of business the spec does not state. When the spec lacks the value, write copy that does not need it instead of inventing one.
