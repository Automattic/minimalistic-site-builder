...
- The contract's ownership split gives identity to the header and the proposition to the hero: when the header already displays the site name or tagline, the H1 must not repeat either verbatim as the headline — the same words twice within one viewport read as a mistake, not a brand gesture. Lead the H1 with the proposition; the name may appear inside a longer headline only when the sentence still works without the header's copy.
