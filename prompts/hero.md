...
- ORIENTATION LABEL — one optional short caption-scale paragraph may precede the H1 when it adds useful category, place or date context. Keep it under 80 characters and do not duplicate the header or standfirst. Leave it out when it adds no information.
- TEXT BUDGET — the copy region holds one H1, at most one optional caption label, at most one supporting paragraph, and the planned action. The standfirst is one sentence, about two set lines and no more than roughly 180 characters. Do not fold overflow into the standfirst. Preserve a clear first-viewport hierarchy.
