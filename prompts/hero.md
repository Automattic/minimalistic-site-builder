...
- TEXT BUDGET — the copy region holds the level-1 headline plus AT MOST the recipe's stated number of supporting paragraphs (one for a compact-capacity recipe, two for a standard-capacity recipe), and at most one planned button. Never stack a further caption line, credit line, or second standfirst: when the copy does not fit the budget, fold it into the standfirst or cut it.
