...
- The hero H1 always authors `"fontSize":"display"`; `display` is the masthead token and no lower heading preset belongs on this one headline. Do not step down to `section-title` or `heading` for a narrow copy rail. A deterministic finish pass bounds the rendered display token to the copy measure and blueprint line target when necessary, preserving the committed display scale without allowing a long word to overflow.
