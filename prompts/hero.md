...
- The contract's ownership split gives identity to the header and the proposition to the hero: when the header already displays the site name, the H1 must not repeat that name verbatim as the headline — the name directly above its own echo reads as a mistake, not a brand gesture. Lead the H1 with the proposition; the site name may appear inside a longer headline only when the sentence still works without the header's copy.
