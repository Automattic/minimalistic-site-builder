...
- Sit tight under the header: when the recipe leads with media, the media is the section's first visual element and the root group's top padding is at most the `sm` spacing preset; large breathing room belongs inside the composition, not between the header and the hero.
...
- EYEBROW GATE — the contract's `header.text_rows` decides whether an eyebrow/kicker may exist at all. When it is 2, the header already stacks a second small text line (a tagline or a nav row) directly above this section: do NOT open with an eyebrow — the level-1 headline is the copy region's FIRST text line, and any orientation copy (place, dates, category) moves into the standfirst below it. Two caption-scale tracked lines ~100px apart read as a rendering bug, not hierarchy. An eyebrow is available only when `header.text_rows` is 1, and even then it is optional — a hero that opens straight on the headline is always valid.
...
- PROTECTION DIM IN TOKEN FORM — when the contract assigns a `top_protection_token` to this opening and the composition dims its cover, author the dim as `"dimRatio"` of at least 40 with `"overlayColor":"<that token slug>"`. Never spell the same color as `customOverlayColor` hex or substitute a gradient for the dim: the build verifies the overlay header against the exact token, and an unverifiable dim silently costs the whole site its overlay header.
