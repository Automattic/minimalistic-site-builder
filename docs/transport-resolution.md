...
A build can be triggered many ways — the CLI, Studio, or a request *from inside a coding agent* (Claude Code, Codex, OpenCode, pi.dev). Something has to decide **which `Llm` to inject** into the pipeline, and therefore into every step. A step never chooses its transport; it calls the injected `$this->llm`. So correctness reduces to one thing: **the entry point resolves the right transport once and injects it everywhere.** `build_pipeline(Llm $llm)` hands that single instance to every step and the `ConcurrentGroup`, so there is no per-step drift — the whole build shares one transport (only the *model* varies per step, via `step_models()`).
