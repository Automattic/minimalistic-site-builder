...
/** Composer-installed "version@commit" for a package, or "unknown" off-Composer. */
function transformer_provenance(string $package): string
{
    if (!class_exists(\Composer\InstalledVersions::class)
        || !\Composer\InstalledVersions::isInstalled($package)
    ) {
        return 'unknown (not Composer-installed)';
    }
    ...
    return $reference === '' ? $version : $version . '@' . substr($reference, 0, 12);
}
 ...
 */
function spike_markdown(array $report, array $details, string $status, string $transformerProvenance): string
{
    $lines = [
        ...
        '',
        '- Transformer: `' . $transformerProvenance . '`',
...
try {
    ...
    // Provenance for the report: the Composer-installed transformer version and
    // commit (replaces the old vendored UPSTREAM_COMMIT file).
    $transformerProvenance = transformer_provenance('automattic/blocks-engine-php-transformer');
    ...
    $markdown = spike_markdown($report, $details, $result->status, $transformerProvenance);
