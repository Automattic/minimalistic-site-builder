...
## Iteration counter
- **23** (2026-08-10, Fix iteration — BIGR-800; rebuild side-signal: all 3 naturaleza rebuilds planned 5-6 sections incl. real menu/reservations sections — watch whether BIGR-801's thin-plan class still reproduces before investing its fix). Rebuilds naturaleza7-9 kept in projects/ as evidence.
...
| 2 | **BIGR-798** | pulso2 desktop H1 "ELECTRONI/C" mid-word snap; display clamp max 104px vs 640px copy measure. Fixed by HeroHeadlineFit in HeaderHeroStep: longest-word width = char count × one per-character em constant (no per-glyph font table), measured against the layout chain (innermost px contentSize/flexSize/column width, narrowed by % columns and media-text halves), pinned to `min(var(--display), <cap>px)`; `overflow-wrap: break-word` stays the last resort. Preset attr+class dropped since core renders has-*-font-size with !important. **Rejected alternative (measured 2026-08-10, portfolio4/pulso5):** a `cqi` ceiling with `container-type: inline-size` on `.hero-composition__copy` collapses the hero copy column to 0 — core gives a cover with custom `contentPosition` `width:auto` on its inner container, so the column is content-sized and size containment zeroes the contained child's contribution. Do not reintroduce container queries on generated boxes. Evidence gists 78b02439043f39d163cc82101f3c1419 (original) + cadf901dc635a0466eedc5727fb7422f (revision: word fit, tbilisi5 hyphenation, container-query collapse); replay pulso22 in projects/. | 1/7 P0 | deterministic | **pr-open #250** |
| 3 | **BIGR-799** | validator misreads EVERY overlay header as "stacked" — `header-overlay` is a legacy token HeaderHeroStep strips (HeaderFallback still emits it). Fixed: mode derives from header-behavior-overlay-to-solid / header-start-transparent / legacy fallback token. Replay: portfolio61 false row gone, hearth21 control clean. | 4/7 warnings | deterministic | **pr-open #252** |
| 4 | **BIGR-800** | primary-action retarget keeps stale label. Fixed at the root: normalize() now REJECTS same-page and path-qualified CTA fragments naming no planned section on their target page, so the model's one repair round reconciles sections+destination+label together (smallest-unit retarget/removal stays as the repeated-failure backstop; bare '#' still passes). 3/3 naturaleza rebuilds: all CTAs resolve, labels honest, 0 backstop retargets. Gist 695bd92a90519b7afe9557a6fb2f4ae4. | 1/7 | plan validation | **pr-open #253** |
