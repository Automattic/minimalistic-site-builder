...
| 4 | **BIGR-800** | primary-action retarget keeps stale label. Fixed at the root: normalize() now REJECTS a same-page CTA fragment naming no planned section, so the model's one repair round reconciles sections+destination+label together (retarget stays as backstop, bare '#' still passes). 3/3 naturaleza rebuilds: all CTAs resolve, labels honest, 0 backstop retargets. Gist 695bd92a90519b7afe9557a6fb2f4ae4. | 1/7 | plan validation | **pr-open #253** |
