...
test('custom-motion removes ungrounded generated content and keeps safe motion', function () {
    [$project, $tmp] = cm_project('builder_cm_contact_content_');
    $project->writeText(
        'theme/parts/section-hero.html',
    ...
    $llm = new FakeLlm();
    $llm->queueText(
        '.custom-motion::before { content: "fake" "@" "example.com"; transform: translateY(1px); }',
    );

    quietly(fn () => (new CustomMotionStep($llm, new PromptRenderer(repo_path('prompts'))))->run($project));

    $style = $project->readText('theme/style.css');
    assert_true(!str_contains($style, 'fake@example.com'));
    assert_contains('transform: translateY(1px)', $style);
    $warnings = implode("\n", $project->readJson('warnings.json')['custom-motion'] ?? []);
    assert_contains('removed_ungrounded_contact', $warnings);
    assert_contains("block='generated custom-motion CSS'", $warnings);
    exec('rm -rf ' . escapeshellarg($tmp));
});

test('custom-motion replaces its previous generated tail before a rerun', function () {
    [$project, $tmp] = cm_project('builder_cm_fixed_point_');
    $project->writeJson('siteSpec.json', [
        'name' => 'Demo',
        'animation_request' => 'the logo should spin on hover',
    ...
    $llm = new FakeLlm();
    $llm->queueText('.custom-motion::before { content:"fake\\40 example.com"; transform:none; }');
    $llm->queueText('.custom-motion { transform:translateY(1px); }');
    $step = new CustomMotionStep($llm, new PromptRenderer(repo_path('prompts')));
    ...
    $style = $project->readText('theme/style.css');
    assert_eq(1, substr_count($style, 'generated by the custom-motion step'));
    assert_true(!str_contains($style, 'fake\\40 example.com'));
    assert_contains('translateY(1px)', $style);
    exec('rm -rf ' . escapeshellarg($tmp));
});

test('custom-motion does not mistake an authored marker string for its owned tail', function () {
    [$project, $tmp] = cm_project('builder_cm_marker_collision_');
    $project->writeText(
        'theme/parts/section-hero.html',
        '<!-- wp:group {"className":"custom-motion"} --><div class="wp-block-group custom-motion">'
            . '<p>Card</p></div><!-- /wp:group -->',
    );
    $authored = "/* Theme Name: Demo */\n"
        . '.x::before{content:"/* Custom motion — user-requested animation, generated by the '
        . 'custom-motion step. */";color:red}' . "\n.keep{color:blue}\n";
    $project->writeText('theme/style.css', $authored);
    $llm = new FakeLlm();
    $llm->queueText('.custom-motion { transform:translateY(1px); }');
    ...
    $style = $project->readText('theme/style.css');
    assert_contains($authored, $style);
    assert_contains('.keep{color:blue}', $style);
    assert_eq(2, substr_count($style, 'generated by the custom-motion step'));
    exec('rm -rf ' . escapeshellarg($tmp));
});
