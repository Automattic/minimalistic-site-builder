...
test('an orphan project tile after a two-column row spans the full row (frm PR-3au)', function () {
    ...
    // The block JSON escapes the literal `--` (illegal inside an HTML comment).
    ...
    // The serializer that runs after every unit rewrites the flex-basis and the class from the attributes.
    ...
    // Idempotent, and the pass is limited to the archetype and to an orphan after a pair.
    $again = [];
