 ...
 * css2 URL builder, the fonts.php validator (scan floor, safe PHP, Google-only
...
test('googleFontsUrl builds the css2 axis forms', function () {
    assert_eq(
        ...
        FontsPhpStep::googleFontsUrl(['Oswald'], [300, 400, 700], false)
    );
    assert_eq(
        'https://fonts.googleapis.com/css2?family=Source+Serif+4:ital,wght@0,400;0,700;1,400;1,700&display=swap',
        FontsPhpStep::googleFontsUrl(['Source Serif 4'], [400, 700], true)
    );
});

test('validate accepts a well-formed fonts.php that covers the scan', function () {
    // Exactly the scan.
    $php = fp_valid_php(FontsPhpStep::googleFontsUrl(['Oswald'], [300, 400, 700], false));
    assert_eq([], FontsPhpStep::validate($php, ['Oswald'], [300, 400, 700], false));
    ...
    assert_eq([], FontsPhpStep::validate($php, ['Oswald'], [400, 700], false));
});
...
test('validate enforces the scan as a floor', function () {
    ...
    assert_true([] !== FontsPhpStep::validate($php, ['Oswald'], [300, 400, 700], false), 'missing weight');
    // Scanned italics missing.
    assert_true([] !== FontsPhpStep::validate($php, ['Oswald'], [400, 700], true), 'missing italics');
    // A family missing entirely.
    assert_true([] !== FontsPhpStep::validate($php, ['Oswald', 'Source Serif 4'], [400, 700], false), 'missing family');
});

test('validate rejects unsafe or off-contract PHP', function () {
    $url = FontsPhpStep::googleFontsUrl(['Oswald'], [400, 700], false);
    foreach ([
        ...
        'forbidden call' => fp_valid_php($url) . "\nfile_get_contents('/etc/passwd');\n",
        'nested require' => fp_valid_php($url) . "\nrequire_once __DIR__ . '/more.php';\n",
        'superglobal' => fp_valid_php($url) . "\n\$x = \$_GET['x'];\n",
        'not php' => "add_action('enqueue_block_assets', fn () => null);",
        'syntax error' => fp_valid_php($url) . "\nfunction broken( {\n",
    ] as $label => $php) {
        assert_true([] !== FontsPhpStep::validate($php, ['Oswald'], [400, 700], false), $label);
    }
});
...
test('run writes the model fonts.php when valid and passes the configured model', function () {
    ...
    // Valid model output covering the scan {300, 400, 500, 700 + italics}.
    $url = FontsPhpStep::googleFontsUrl(['Cormorant Garamond'], [300, 400, 500, 700], true);
    $llm = new FakeLlm();
    ...
    assert_contains('Weights: 300, 400, 500, 700', $llm->calls[0]['prompt']);
    assert_contains('Italics: yes', $llm->calls[0]['prompt']);
...
test('run falls back to the deterministic fonts.php when the model output fails', function () {
    ...
    $php = $project->readText('theme/fonts.php');
    assert_contains('family=Oswald:wght@300;400;700', $php, 'fallback requests the scanned weights');
    assert_contains("add_action('enqueue_block_assets'", $php);
    assert_contains('scanned weight not requested: 300', $project->readText('logs/fonts-php.log'));
    exec('rm -rf ' . escapeshellarg($tmp));
});

test('run skips (no LLM call, no file) when only system families are named', function () {
    [$project, $tmp] = fp_project([
