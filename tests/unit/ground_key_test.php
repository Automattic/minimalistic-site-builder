...
test('a direct negative ground instruction cannot override a positive page instruction', function () {
    foreach (['Avoid a dark background.', 'Do not use a dark page.', "Don't use a dark background.", 'No dark mode.'] as $negative) {
        assert_eq('light', GroundKey::statedInBrief($negative . ' Use a white page.'));
    }
    assert_eq('dark', GroundKey::statedInBrief('Use a dark page.'));
});
