...
test('a cardless archetype releases its planned item pattern and corrects the notes (frm PR-3v)', function (): void {
    ...
    $repairs = [];
    $delivered = PagePlanStep::reconcileRecipeItemPatterns($pages, $repairs);
    ...
    $fixedPointRepairs = [];
    assert_eq($delivered, PagePlanStep::reconcileRecipeItemPatterns($delivered, $fixedPointRepairs));
...
test('card recipes keep cards under a ruled site pattern', function (): void {
    foreach (['bento-grid', 'pricing-tiers'] as $archetype) {
        ...
        $repairs = [];
        $out = PagePlanStep::reconcileRecipeItemPatterns($pages, $repairs);
        ...
        $again = [];
        assert_eq($out, PagePlanStep::reconcileRecipeItemPatterns($out, $again));
        assert_eq([], $again);
    }
});
