...
test('a stated accent anchors the monochrome palette', function () {
    $palette = ['base' => '#FFFFFF', 'contrast' => '#111111', 'primary' => '#2244AA', 'secondary' => '#4455BB', 'accent' => '#E87416', 'band' => '#E5E5E5'];
    $warnings = [];
    $out = PaletteFloor::repair($palette, $warnings, null, 'monochrome', true);
    assert_eq('#E87416', $out['accent']);
    assert_true(abs(PaletteFloor::hue($out['primary']) - PaletteFloor::hue($out['accent'])) < 2);
    $again = [];
    assert_eq($out, PaletteFloor::repair($out, $again, null, 'monochrome', true));
    assert_eq([], $again);
});
