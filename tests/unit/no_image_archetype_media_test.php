...
test('a zero-media archetype drops its authored image blocks at the block boundary and keeps the copy (frm PR-3u)', function () {
    ...
    $repairs = [];
    $warnings = [];
    $out = GeneratedMarkup::stripMediaOffNoImageArchetype($markup, 'page-home--services', 'logo-strip', $repairs, $warnings);
    ...
    assert_contains('section-composition--logo-strip', $out);
    assert_eq(1, count($repairs));
    assert_eq('no-image-archetype-media-removed', $repairs[0]['code']);
    assert_eq('2 authored image(s) on logo-strip', $repairs[0]['authored']);
    assert_eq('removed', $repairs[0]['delivered']);
    assert_eq(2, count($warnings));
    assert_contains('the logo-strip archetype plans no media', $warnings[0]);
    assert_contains('service-research-sphere', $warnings[0]);
    $joined = implode("\n", SectionComposition::markupWarnings($out, 'logo-strip', 'page-home--services'));
...
test('every zero-media archetype strips, a media archetype and an unassigned section keep their pictures (frm PR-3u)', function () {
    ...
    foreach (['pricing-tiers', 'stat-ledger', 'feature-row-hairlines', 'logo-strip'] as $archetype) {
        assert_eq(0, (int) SectionComposition::metadata($archetype)['max_images'], $archetype . ' plans no media');
    }
    $markup = no_image_band('logo-strip', no_image_picture('object'));
    $repairs = [];
    $warnings = [];
...
test('a section over its media budget keeps the first pictures in document order and drops the rest (frm PR-3y)', function () {
    ...
    $three = $band('cta-panel', no_image_picture('screen-one') . '<!-- wp:paragraph --><p>Copy.</p><!-- /wp:paragraph -->' . no_image_picture('screen-two') . no_image_picture('screen-three'));
    $repairs = [];
    $warnings = [];
    $out = GeneratedMarkup::stripMediaOverBudget($three, 'page-home--dashboard', 'cta-panel', $repairs, $warnings);
    assert_eq(1, preg_match_all('~<img\\b~', $out), 'a cta panel has one supporting image');
    ...
    assert_eq('3 authored image(s) on cta-panel (budget 1)', $repairs[0]['authored']);
    ...
    $againRepairs = [];
    $againWarnings = [];
    assert_eq($out, GeneratedMarkup::stripMediaOverBudget($out, 'page-home--dashboard', 'cta-panel', $againRepairs, $againWarnings));
    assert_eq([], $againRepairs);
    assert_eq([], $againWarnings);
    $joined = implode("\n", SectionComposition::markupWarnings($out, 'cta-panel', 'page-home--dashboard'));
    assert_true(!str_contains($joined, 'archetype media count'), 'the media count check is satisfied');
    ...
    $within = $band('cta-panel', no_image_picture('a'));
    $repairs = [];
    $warnings = [];
    assert_eq($within, GeneratedMarkup::stripMediaOverBudget($within, 'page-home--x', 'cta-panel', $repairs, $warnings));
    assert_eq([], $repairs);
    assert_eq($within, GeneratedMarkup::stripMediaOverBudget($within, 'page-home--x', null, $repairs, $warnings));
    $zero = $band('logo-strip', no_image_picture('a'));
    assert_eq($zero, GeneratedMarkup::stripMediaOverBudget($zero, 'page-home--x', 'logo-strip', $repairs, $warnings), 'a zero budget belongs to the other repair');
    assert_eq([], $repairs);
});
...
test('media removal preserves a block with other content and removes only the independent image', function () {
    ...
    $markup = no_image_band('logo-strip', $preserved . no_image_picture('remove'));
    $repairs = [];
    $warnings = [];
    $out = GeneratedMarkup::stripMediaOffNoImageArchetype($markup, 'page-home--values', 'logo-strip', $repairs, $warnings);
    ...
    $againRepairs = [];
    $againWarnings = [];
    assert_eq($out, GeneratedMarkup::stripMediaOffNoImageArchetype($out, 'page-home--values', 'logo-strip', $againRepairs, $againWarnings));
});
