...
/** Foundation chunk that neutralizes a doubled chrome landmark's box model. */
function ps_nested_landmark(): string
{
...
test('legacy mode ignores stale site CSS and keeps the recorded call trace and style bytes', function () {
    ...
    assert_eq(
        '728829a94fdf4c861063a7b521b24a85b3f4f33831081fa7e3700ba2523b548c',
...
test('site CSS path always merges a wrap policy that never splits words', function () {
    ...
    $style = $project->readText('theme/style.css');
    assert_eq(
        $scaffold . ps_wrap() . ps_table_reset() . ps_nested_landmark() . ps_baseline('home'),
        $style,
        'wrap, table-reset, nested-landmark and heading-baseline foundation is the whole tail when there is no design CSS'
    );
...
test('site CSS path warns for every scrub removal and continues after an empty source', function () {
    ...
    $base = $project->readText('theme/style.css') . ps_wrap() . ps_table_reset() . ps_nested_landmark() . ps_baseline('about');
