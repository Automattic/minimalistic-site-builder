...
test('page-plan mechanical repairs keep the authored copy and reach a fixed point', function () {
    ...
    foreach ($cases as $case => $archetypes) {
        ...
        $again = $case === 'field'
            ...
            : PagePlanStep::repairLayoutCompatibility($out, warnings: $againWarnings, pageSlug: 'home', allowOffsetGrid: false);
...
test('page-plan mechanical repairs leave a legal plan untouched', function () {
    ...
    $warnings = [];
    $out = PagePlanStep::repairFields($sections, $warnings);
    $out = PagePlanStep::repairLayoutCompatibility($out, warnings: $warnings);
    assert_eq($sections, $out);
    assert_eq([], $warnings);
});

test('page-plan leaves a valid short opening alone instead of padding a generic close', function () {
    $hero = plan_section();
    $warnings = [];
    $out = PagePlanStep::repairLayoutCompatibility([$hero], warnings: $warnings);
    assert_eq([$hero], $out);
    assert_eq([], $warnings);
});
...
test('a repeated list planned as an asymmetric split takes the card grid (frm PR-3aq)', function () {
    ...
    assert_eq('equal-card-grid', $a['team'], 'a third grid remains valid without a page quota');
    ...
    assert_eq('equal-card-grid', array_column($out[0]['sections'], 'layout_archetype', 'slug')['services'], 'adjacent grids are allowed');
    assert_eq(1, count($repairs));
