...
test('numeral removal and renumber reach a fixed point and preserve sibling content', function () {
    $markup = step_numeral_section(step_numeral_item('Step 1') . step_numeral_item('7'));
    $result = StepNumeral::normalize($markup, 'chip', 'page-home--process', true);
    assert_contains('>1</p>', $result['markup']);
    assert_eq(1, count($result['warnings']));
    assert_eq(1, count($result['repairs']));
    foreach (['file=', 'block=', 'authored=', 'delivered=removed', 'disposition='] as $context) {
        assert_contains($context, $result['warnings'][0]);
    }
    $again = StepNumeral::normalize($result['markup'], 'chip', 'page-home--process', true);
...
test('explicit numbered rows commit a numeral and match only section identity', function () {
    ...
    $repairs = [];
    assert_eq('chip', DesignDirectionStep::withStatedStepNumeral(['step_numeral' => 'chip'], $meta, $repairs)['step_numeral']);
    assert_eq([], $repairs);
    assert_true(StepNumeral::clauseAppliesTo(StepNumeral::statedNumberedFor($meta), ['slug' => 'features']));
